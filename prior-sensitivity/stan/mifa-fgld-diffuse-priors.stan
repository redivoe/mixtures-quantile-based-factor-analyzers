...
model {
  ...
  for(k in 1:K){
    mu[k] ~ normal(0, 10);
    to_vector(Lambda[k]) ~ normal(0, 10);
    psi[k] ~ exponential(0.01);
    delta[k] ~ uniform(0, 1);
    kappa[k] ~ exponential(0.1);
    for(j in 1:q){
      beta[k][j] = sqrt(1 / h1(delta[k][j], kappa[k][j]));
      alpha[k][j] = - beta[k][j] * (2 * delta[k][j] - 1 + kappa[k][j] / 2);
      Y[k][, j] = fgldvec_qf(U[, j], to_vector({alpha[k][j], beta[k][j], delta[k][j], kappa[k][j]}));
    }
